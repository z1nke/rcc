...
VarDecl *VarDecl::create(ASTContext &Ctx, SourceLocation BegLoc,
  ...
  return new (Mem) VarDecl(Ctx, BegLoc, EndLoc, T, std::move(Name));
}

VarDecl::VarDecl(ASTContext &Ctx, SourceLocation BegLoc, SourceLocation EndLoc,
                 QualType T, std::string Name)
    : ValueDecl(DK_Var, BegLoc, EndLoc, T), Ctx(Ctx), Name(std::move(Name)) {}
...
FunctionDecl *FunctionDecl::create(ASTContext &Ctx, SourceLocation BegLoc,
  ...
  return new (Mem) FunctionDecl(Ctx, BegLoc, EndLoc, T, Body);
}

FunctionDecl::FunctionDecl(ASTContext &Ctx, SourceLocation BegLoc,
                           SourceLocation EndLoc, QualType T, Stmt *Body)
    : ValueDecl(DK_Function, BegLoc, EndLoc, T), Ctx(Ctx), Body(Body) {}
