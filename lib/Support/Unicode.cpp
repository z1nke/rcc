...
/// Decode one UTF-8 code point without diagnostics. Invalid sequences consume
/// one byte and return that byte value (width 1).
static std::uint32_t decodeUTF8Lenient(const char **NewPos, const char *P) {
  if (static_cast<unsigned char>(*P) < 128) {
    *NewPos = P + 1;
    return static_cast<unsigned char>(*P);
  }
...
// Columns needed for one code point in a fixed-width font.
// Based on https://www.cl.cam.ac.uk/~mgk25/ucs/wcwidth.c.
static int charWidth(std::uint32_t C) {
  // clang-format off
  static const std::uint32_t Range1[] = {
  ...
  // clang-format off
  static const std::uint32_t Range2[] = {
...
int displayWidth(const char *P, int Len) {
  ...
  int W = 0;
  while (P - Start < Len) {
    std::uint32_t C = decodeUTF8Lenient(&P, P);
    W += charWidth(C);
  }
  return W;
}
